"""HUD V2 - Phase 3 - Arc Reactor + Live Transcription"""
...
class ArcReactorHUD(QWidget):
    """Arc Reactor HUD - Glowing ring + live transcription + system stats"""
    ...
    def __init__(self, parent=None):
        ...
        logger.info("ArcReactorHUD V2 - Phase 3")
    ...
    def paintEvent(self, event):
        ...
        # Outer glow (arc reactor)
        glow_radius = radius + 20 + 10 * self._glow_val
        for i in range(20):
        ...
        # Transcription around ring (bottom)
        painter.setPen(QColor(255, 255, 255, 200))
        font2 = QFont("Consolas", 10)
        painter.setFont(font2)
        painter.drawText(cx-180, cy+radius+30, 360, 40, Qt.AlignCenter | Qt.TextWordWrap, self._transcription)

        # System stats (top)
        stats_text = f"CPU: {self._system_stats['cpu']:.0f}% | RAM: {self._system_stats['ram']:.0f}% | Mic: {self._system_stats['mic_level']:.3f}"
        painter.drawText(cx-180, cy-radius-40, 360, 20, Qt.AlignCenter, stats_text)
