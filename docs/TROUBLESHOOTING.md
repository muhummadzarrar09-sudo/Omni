...
native OS architecture and uses `--require-hashes` when that lock is present.
Windows locks generated by the resolver include the exact `pyproject.toml` build
backend (`setuptools` and `wheel`); the installer then builds local source with
...
resolves all six profiles twice, compares the exact locks, installs, reinstalls,
starts twice, restarts, performs authenticated readiness, stops, safely
uninstalls, explicitly removes isolated data, repeats uninstall, runs Python,
package, configuration, frontend test/lint/build, and cleanup gates, and writes
the native lane attestation outside the worktree:
